...
rf_grid <- grid_regular(
  ...
  min_n(range = c(2, 10)),     # Zakres dla min_n (np. 2-10)
...
# Wyciągnij najlepsze wyniki dla każdej kombinacji hiperparametrów
results <- collect_metrics(rf_tune_results)

# Stwórz wykres dla metryki roc_auc w zależności od mtry i min_n
ggplot(results, aes(x = mtry, y = mean, color = factor(min_n))) +
  geom_line() +
  geom_point() +
  labs(
    title = "Optymalizacja hiperparametrów modelu lasu losowego",
    x = "Liczba wybranych predyktorów (mtry)",
    y = "Średnia wartość ROC AUC",
    color = "Minimalna liczba próbek w liściu (min_n)"
  ) +
  theme_minimal()
